...
SSL_BACKUP_DIR="$Z_BASE_DIR/ssl/letsencrypt/bak.$TODAY"
...
if ! mkdir -p "$SSL_BACKUP_DIR"; then
	print_v e "   Unable to make backup directory $SSL_BACKUP_DIR" | tee -a "$MESSAGE_FILE.errors" "$LOG_FILE" > /dev/null
	$Z_BASE_DIR/common/sbin/sendmail -t "$EMAIL" < "$MESSAGE_FILE.errors" |& tee -a "$LOG_FILE"
	exit 1
fi
chown zimbra:zimbra "$SSL_BACKUP_DIR"


#Backup Old Cert
print_v i "Backup Old Cert" >> "$LOG_FILE"
if ! cp $Z_BASE_DIR/ssl/letsencrypt/*.pem "$SSL_BACKUP_DIR/"; then
	print_v e "   Unable to backup old Certiricate, stopping" | tee -a "$MESSAGE_FILE.errors" "$LOG_FILE" > /dev/null
	$Z_BASE_DIR/common/sbin/sendmail -t "$EMAIL" < "$MESSAGE_FILE.errors" |& tee -a "$LOG_FILE"
	exit 1
fi
...
if ! cp /etc/letsencrypt/live/"$FQDN"/* $Z_BASE_DIR/ssl/letsencrypt/; then
	print_v e "   Unable to copy new Certiricate to $Z_BASE_DIR/ssl/letsencrypt, stopping" | tee -a "$MESSAGE_FILE.errors" "$LOG_FILE" > /dev/null
	$Z_BASE_DIR/common/sbin/sendmail -t "$EMAIL" < "$MESSAGE_FILE.errors" |& tee -a "$LOG_FILE"
	exit 1
fi
...
if ! wget -o /tmp/ISRG-X1.pem.log -O /tmp/ISRG-X1.pem $X1CERTURI; then
	print_v w "Unable to download X1 Cross Signed Cert" | tee -a "$MESSAGE_FILE.errors" "$PROGRESS_FILE" > /dev/null
...
if [[ $? -ne 0 ]]; then
	...
	print_v e "'zmproxyctl stop' command failed" | tee -a "$LOG_FILE" "$PROGRESS_FILE" > /dev/null
	exit 1
fi

print_v d "About to run 'zmmailboxdctl stop'"
print_v i "About to run 'zmmailboxdctl stop'" | tee -a "$LOG_FILE" "$PROGRESS_FILE" > /dev/null
sudo -u zimbra -g zimbra bash << EOF
	source ~/.bashrc
...
if [[ $? -ne 0 ]]; then
	...
	print_v e "'zmmailboxdctl stop' command failed" | tee -a "$LOG_FILE" "$PROGRESS_FILE" > /dev/null
	exit 1
fi

#backup zimbra certs
print_v d "About to backup Zimbra Certs"
print_v i "About to backup Zimbra Certs" | tee -a "$LOG_FILE" "$PROGRESS_FILE" > /dev/null
...
#Deploy Certificate
NOW_DATE=$(date)
print_v i "About to Deploy 'zmcertmgr deploycrt comm at $NOW_DATE'" | tee -a "$LOG_FILE" "$PROGRESS_FILE" > /dev/null
print_v d "Running: $Z_BASE_DIR/bin/zmcertmgr deploycrt comm $Z_BASE_DIR/ssl/letsencrypt/cert.pem $Z_BASE_DIR/ssl/letsencrypt/chain.pem"
sudo -u zimbra -g zimbra bash << EOF
	source ~/.bashrc
...
# shellcheck disable=SC2181
if [[ $? -ne 0 ]]; then
	print_v e "'certmgr deploycrt comm' command failed" | tee -a "$MESSAGE_FILE.errors"
	$Z_BASE_DIR/common/sbin/sendmail -t "$EMAIL" < "$MESSAGE_FILE.errors" |& tee -a "$LOG_FILE"
	exit 1
else
	print_v d "'certmgr deploycrt comm' command success"
	print_v i "'certmgr deploycrt comm' command success" | tee -a "$LOG_FILE" "$PROGRESS_FILE" > /dev/null
fi
...
print_v i "Print_v: About to restart 'zmcontrol restart' at $NOW_DATE" | tee -a "$LOG_FILE" "$PROGRESS_FILE" > /dev/null
restart_zimbra
print_v i "Print_v: Command Complete 'zmcontrol restart' at $NOW_DATE" | tee -a "$LOG_FILE" "$PROGRESS_FILE" > /dev/null

echo "-Testing Echo 618---" >> "$PROGRESS_FILE"

#Restarting Zimbra above should mean you don't need to restart proxyctl
#restart_zimbra_proxy

print_v i "All done. About to send message of completion" | tee -a "$LOG_FILE" "$PROGRESS_FILE" > /dev/null
echo "-Testing Echo 624---" >> "$PROGRESS_FILE"
$Z_BASE_DIR/common/sbin/sendmail -t "$EMAIL" < "$PROGRESS_FILE" |& tee -a "$LOG_FILE"
...
print_v i "Check 0: About to sleep for 25 seconds to check if zimbra is running" | tee -a "$LOG_FILE" "$PROGRESS_FILE" > /dev/null
sleep 25
print_v i "Check 0: About to run 'restart_zimbra_if_not_running' " | tee -a "$LOG_FILE" "$PROGRESS_FILE" > /dev/null
restart_zimbra_if_not_running
print_v i "Check 0: Command Complete 'restart_zimbra_if_not_running' " | tee -a "$LOG_FILE" "$PROGRESS_FILE" > /dev/null

#Email progress report of check 0
print_v i  "sendmail of progress report" >> "$LOG_FILE"
$Z_BASE_DIR/common/sbin/sendmail -t "$EMAIL" < "$PROGRESS_FILE" |& tee -a "$LOG_FILE"

#Is 25 seconds enough? Perhaps not. Let's check several more times if we aren't in DEBUG mode
if [[ $DEBUG == 0 ]]; then
	for CHECKNUM in $(seq 1 3); do 
		#Do we believe that all services are running? Let's check again
		print_v i "Check $CHECKNUM: About to sleep for 10 minutes to check " | tee -a "$LOG_FILE" "$PROGRESS_FILE" > /dev/null
		sleep 600
		print_v i "Check $CHECKNUM: About to run 'restart_zimbra_if_not_running' " | tee -a "$LOG_FILE" "$PROGRESS_FILE" > /dev/null
		restart_zimbra_if_not_running
		print_v i "Check $CHECKNUM: Command Complete 'restart_zimbra_if_not_running' " | tee -a "$LOG_FILE" "$PROGRESS_FILE" > /dev/null
	done
fi

print_v i "--About to exit program and email progress file" | tee -a "$LOG_FILE" "$PROGRESS_FILE" > /dev/null
$Z_BASE_DIR/common/sbin/sendmail -t "$EMAIL" < "$PROGRESS_FILE" |& tee -a "$LOG_FILE"
print_v i "--Sent last email, exiting" | tee -a "$LOG_FILE" "$PROGRESS_FILE" > /dev/null
